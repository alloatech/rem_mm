-- Fix get_user_leagues to bypass RLS policies
-- The function was missing SECURITY DEFINER, causing RLS to block results
...
LANGUAGE SQL
STABLE
SECURITY DEFINER  -- Run with definer's (postgres) privileges, bypassing RLS
AS $$
  SELECT 
    l.id,
    l.sleeper_league_id,
